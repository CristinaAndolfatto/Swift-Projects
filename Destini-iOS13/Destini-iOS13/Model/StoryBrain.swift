...
struct StoryBrain {
    // var that indicates the story number
    var storyNumb = 0
    // array that contains the stories
    let stories = [
    ...
    // Method to update the number of choices made
    mutating func newStory(userChoice : String) {
            // check of the answer: if the User choose choice1, go to tigerStory
            if userChoice == stories[storyNumb].choice1 {
                storyNumb = stories[storyNumb].choice1Destination
            }
            else if userChoice == stories[storyNumb].choice2 {
                // go to treasure story
                storyNumb = stories[storyNumb].choice2Destination
            }
    }
    
    func getStoryTitle() -> String {
        return stories[storyNumb].title
    }
    
    func getChoiceOne() -> String {
        return stories[storyNumb].choice1
    }
    
    func getChoiceTwo() -> String {
        return stories[storyNumb].choice2
    }
